...
a pool of previous states. We track the final image of each item in the pool,
restricting the states it has to persist in.
"""
import argparse
import random
...
def main():
        ...
        with torch.no_grad():
            ...
            if args.t1 is None:
                ...
                for i in tqdm(range(max_train)):
                    ...
                    plt.savefig(f"temp/generated/dna_persist/{i}.png")
            ...
            # Transitioning
                ...
                for i in tqdm(range(max_train)):
                    ...
                    plt.savefig(f"temp/generated/dna_persist/{max_train + i}.png")
        quit()
    ...
    pool_finals = np.array([random.randrange(0, final_img_arrs.shape[0]) for _ in range(pool_size)])
    for i in tqdm(range(train_iters)):
        ...
        img_idxs = pool_finals[pool_idxs]
        final = torch.from_numpy(final_img_arrs[img_idxs]).to(device)
        dna = (
            enc_net(final)
            .reshape(iter_batch_size, dna_size, 1, 1)
            .repeat(1, 1, sim_size, sim_size)
